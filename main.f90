...
program main
	...
	!call read_file_meshed_comsol_shift()	
	!call read_file_meshed_comsol()	
	!!
	call initalize_calculation()	
    call test_conical_illumination(p_obj)
	...
	subroutine set_calculation_parameters()
		...
		if (calc_p(1) .eq. 3) then
			...
			illumination_p%Nt = 21
			illumination_p%Np = 19
            illumination_p%phi_max = 0.5*PI
		end if
		...
		if ((calc_p(5) .ge. 3) .and. (calc_p(5) .le. 6))then 
			calc_p(7) = 2
		end if
		...
		!eps_r2_main = (-18.78637, -9.3616) ! Nickel at 500nm
	...
	subroutine set_structure_and_observation_parameters()	
		...
		!3- 'surface' !grating meshed by COMSOL
			...
			geometry_p(1) = 0.5e-6 !grating width
			geometry_p(2) = 0.19e-6 !grating height
		...
		!'surface' !cylinder meshed by COMSOL
			...
			geometry_p(1) = 0.5e-6 !radius
			geometry_p(2) = 1.0e-6 !length of the cylinder
			surface_length = 2*geometry_p(1) !
		else if (calc_p(5) .eq. 6)then
			geometry_p(1) = 500e-9 !periodicity
			geometry_p(2) = 120.0e-9 !PV value of z
			
			surface_length = 21*geometry_p(1) !	Lx
		...
		if (calc_p(4) .le. 3)then
			...
			sampling_nb = 301
			scaling_f = 0.5		
			dim_a_min = -3.0e-6!-surface_length*scaling_f
			dim_a_max = 3.0e-6! surface_length*scaling_f
			dim_b_min = -3.0e-6!-surface_length*scaling_f
			dim_b_max = 3.0e-6!surface_length*scaling_f	
	...
	subroutine set_illumination_parameter()
		...
		if (calc_p(8) .eq. 1)then !TM-polarization
		...
		call field_rotation_matrix(theta_in, phi_in, Ein_p, illumination_p%E_in)
  ...
  !      !when only the test input field is calculated:
	...
	subroutine field_rotation_matrix(theta, phi, Ein_pol, E_in)
