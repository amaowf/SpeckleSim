...
program main
	...
	contains
		...
		if (calc_p(1) .eq. 2) then		
			beam_waist = 2.0e-6 !waist radius
		...
		calc_p(5) = 2 !does not influence far field calculation		
		...
		!environment is air
		!eps_r2_main = (-17.2, -0.498)! (-16.075, -0.442342)!  (2.25, 0.0)!
	...
	subroutine set_structure_and_observation_parameters()
		...
		if (calc_p(4) .ge. 4)then
			sampling_na = 501
			sampling_nb = 1
			theta_start = -90.0/180.0*PI 
			theta_end = 90.0/180.0*PI 
			phi_start = 0.0 
			phi_end = 0.0
		else 	
		!For the case calc_p(4) = 1-3		
			sampling_na = 201
			sampling_nb = 201
		end if
