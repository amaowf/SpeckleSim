...
module lib_sie_lookup_table
  ...
  type LT3_t
     ...
     real(dp)              :: dx(3)       !< The x-spacing in LT3_create_from_datathe lookup coordinate
...
contains
  ...
  ! then the value becomes the value at the right side of D
  ...
  elemental	function LT3_get_loc(my_lt, x1, x2, x3) result(my_loc)!
    ...
    my_loc%low_ix   = ceiling(frac)	 
    my_loc%low_frac = (real(my_loc%low_ix, dp) - frac)
  ...
  !> Get the value of a single column at x
  pure function LT3_get_col(my_lt, col_ix, x1, x2, x3) result(col_value)
    type(LT3_t), intent(in) :: my_lt
    integer, intent(in)     :: col_ix
    real(dp), intent(in)    :: x1, x2, x3
    real(dp)                :: col_value
  ...
  !> Get the value of a single column at a location
  pure  function LT3_get_col_at_loc(my_lt, col_ix, loc) result(col_value) !
    type(LT3_t), intent(in)     :: my_lt
    integer, intent(in)         :: col_ix
...
!> Added by Fu
!> For the cubes in the domain_i4 by lib_sie_mlfmm
  function LT3_create_spaced_data(n_points, n_cols)result(spaced_data)
		integer, intent(in) :: n_points(3), n_cols
		real(dp), dimension(:, :, :, :), allocatable :: spaced_data
		integer :: j, k, t, i
		allocate(spaced_data(n_points(1), n_points(2), n_points(3), n_cols))
		
		!Possible cubes in E4
		do i = 1, n_points(1)
			do j = 1, n_points(2)
				do k = 1, n_points(3)
					t = k + (j-1)*n_points(2) + (i-1)*n_points(2)*n_points(1)
					spaced_data(i, j, k, n_cols) = t
				end do
			end do
		end do
		
		!For cubes in E2
		do i = (n_points(1)-1)/2, (n_points(1)-1)/2+2			
			do j = (n_points(2)-1)/2, (n_points(2)-1)/2+2
				do k = (n_points(3)-1)/2, (n_points(3)-1)/2+2					
					t = k + (j-1)*n_points(2) + (i-1)*n_points(2)*n_points(1)
					spaced_data(i, j, k, n_cols) = 0.0_dp
				end do
			end do
		end do
	end function

end module lib_sie_lookup_table