! reference: https://people.sc.fsu.edu/~jburkardt/f77_src/hermite_polynomial/hermite_polynomial.html

module fsu_hermite_polynomial
    implicit none

    private

    public :: H_INTEGRAL                        ! evaluates the integral of H(i,x).
    public :: H_POLYNOMIAL_COEFFICIENTS         ! coefficients of H(i,x).
    public :: H_POLYNOMIAL_VALUE                ! evaluates H(i,x).
    public :: H_POLYNOMIAL_VALUES               ! tabulated values of H(i,x).
    public :: H_POLYNOMIAL_ZEROS                ! zeros of H(i,x).
    public :: H_QUADRATURE_RULE                 ! quadrature for H(i,x).
    public :: HE_DOUBLE_PRODUCT_INTEGRAL        ! integral of He(i,x)*He(j,x)*e^(-x^2/2).
    public :: HE_INTEGRAL                       ! evaluates the integral of He(i,x).
    public :: HE_POLYNOMIAL_COEFFICIENTS        ! coefficients of He(i,x).
    public :: HE_POLYNOMIAL_VALUE               ! evaluates He(i,x).
    public :: HE_POLYNOMIAL_VALUES              ! tabulated values of He(i,x).
    public :: HE_POLYNOMIAL_ZEROS               ! zeros of He(i,x).
    public :: HE_QUADRATURE_RULE                ! quadrature for He(i,x).
    public :: HE_TRIPLE_PRODUCT_INTEGRAL        ! integral of He(i,x)*He(j,x)*He(k,x)*e^(-x^2/2).
!    public :: HEN_EXPONENTIAL_PRODUCT           ! exponential product exp(b*x)*Hen(i,x)*Hen(j,x).
    public :: HEN_POLYNOMIAL_VALUE              ! evaluates Hen(i,x).
!    public :: HEN_POWER_PRODUCT                 ! power products, x^e*Hen(i,x)*Hen(j,x).
!    public :: HF_EXPONENTIAL_PRODUCT            ! exponential products, exp(b*x)*Hf(i,x)*Hf(j,x).
    public :: HF_FUNCTION_VALUE                 ! evaluates Hf(i,x).
    public :: HF_FUNCTION_VALUES                ! tabulated values of Hf(i,x).
!    public :: HF_POWER_PRODUCT                  ! power products x^e*Hf(i,x)*Hf(j,x).
    public :: HF_QUADRATURE_RULE                ! quadrature for Hf(i,x).
!    public :: HN_EXPONENTIAL_PRODUCT            ! exponential products exp(b*x)*Hn(i,x)*Hn(j,x).
    public :: HN_POLYNOMIAL_VALUE               ! evaluates Hn(i,x).
!    public :: HN_POWER_PRODUCT                  ! power products x^e*Hn(i,x)*Hn(j,x).
    public :: IMTQLX                            ! diagonalizes a symmetric tridiagonal matrix.
    public :: R8_FACTORIAL                      ! computes the factorial of N.
    public :: R8_FACTORIAL2                     ! computes the double factorial function.
    public :: R8_SIGN                           ! returns the sign of an R8.
    public :: R8MAT_PRINT                       ! prints an R8MAT.
    public :: R8MAT_PRINT_SOME                  ! prints some of an R8MAT.
    public :: R8VEC_PRINT                       ! prints an R8VEC.
    public :: R8VEC2_PRINT                      ! prints an R8VEC2.
